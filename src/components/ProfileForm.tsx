...
export default function ProfileForm({ onResult }: ProfileFormProps) {
  ...
  const ic = "w-full bg-surface-800/60 border border-brand-500/20 rounded-xl px-4 py-3 text-white placeholder:text-slate-500 focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400/30 transition-all text-sm";
  ...
  return (
    <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} onSubmit={handleSubmit} className="glass-card p-6 sm:p-8 w-full max-w-2xl mx-auto space-y-5">
      <div className="flex items-center gap-3 mb-2">
        <div className="p-2.5 rounded-lg bg-brand-600/20 border border-brand-500/30"><FiShield className="text-brand-400 text-xl" /></div>
        <div><h2 className="text-xl font-bold text-white">Profile Analysis</h2><p className="text-xs text-slate-500">Enter profile data to verify</p></div>
      </div>
      ...
      <div className="space-y-1.5">
        <label className="flex items-center gap-2 text-sm font-medium text-slate-300"><FiUser className="text-brand-400" /> Username <span className="text-cyber-red">*</span></label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="flex items-center gap-2 text-sm font-medium text-slate-300"><FiUsers className="text-brand-400" /> Followers</label>
        ...
        <div className="space-y-1.5">
          <label className="flex items-center gap-2 text-sm font-medium text-slate-300"><FiFileText className="text-brand-400" /> Posts</label>
          <input type="number" name="posts" value={form.posts} onChange={handleChange} placeholder="0" min={0} className={ic} id="input-posts" />
        </div>
      </div>

      <div className="space-y-1.5">
        <label className="flex items-center gap-2 text-sm font-medium text-slate-300"><FiClock className="text-brand-400" /> Account Age</label>
      ...
      <div className="space-y-1.5">
        <label className="flex items-center gap-2 text-sm font-medium text-slate-300"><FiEdit3 className="text-brand-400" /> Bio</label>
      ...
      <div className="space-y-1.5">
        <label className="flex items-center gap-2 text-sm font-medium text-slate-300"><FiUploadCloud className="text-brand-400" /> Profile Image</label>
        {preview ? (
          <div className="relative inline-block">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={preview} alt="Preview" className="w-24 h-24 rounded-xl object-cover border-2 border-brand-500/30" />
          ...
          <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-brand-500/20 rounded-xl p-6 text-center cursor-pointer hover:border-brand-400/40 transition-colors">
            <FiUploadCloud className="mx-auto text-3xl text-slate-500 mb-2" /><p className="text-xs text-slate-500">Click to upload</p>
          </div>
        )}
